...
content = content.replace(store_replace_1, store_replace_2)

content = content.replace("const activeCompanion = useTrackerStore(state => state.activeCompanion);", "const activeCompanion = useSettingsStore(state => state.activeCompanion);")
...
# Fix bubble logic
bubble_logic_1 = """  let bobX = bobPosition.x;
...
bubble_logic_2 = """  let bobX = companionPosition?.x ?? 800;
...
content = content.replace(bubble_logic_1, bubble_logic_2)

# Fix absolute positioning wrapper
bubble_jsx_1 = """      {/* Tutorial Chat Bubble */}
      <AnimatePresence>
        <motion.div
        ...
        >"""
bubble_jsx_2 = """      {/* Tutorial Chat Bubble */}
...
content = content.replace(bubble_jsx_1, bubble_jsx_2)

bubble_close_1 = """        </motion.div>
      </AnimatePresence>"""
bubble_close_2 = """        </motion.div>
      </AnimatePresence>
      </div>"""

# Only replace the last occurrence of the close block so we don't hit other AnimatePresence
idx = content.rfind(bubble_close_1)
if idx != -1:
    content = content[:idx] + bubble_close_2 + content[idx+len(bubble_close_1):]
