...
['en', 'es', 'ru', 'ko'].forEach(lang => {
    // 1. Inject ui, chestTab, settingsGroup, debug
    const injectStr = Object.keys(newTranslations[lang]).filter(k => k !== 'wizardNew').map(key => {
        return '    "' + key + '": ' + JSON.stringify(newTranslations[lang][key], null, 6).replace(/\n/g, '\n      ') + ',\n';
    }).join('');

    // Let's insert at the very beginning of the language block, right after '"en": {'
    const rx = new RegExp('"' + lang + '":\\s*\\{');
    code = code.replace(rx, '"' + lang + '": {\n' + injectStr);

    // 2. Inject wizardNew into existing 'wizard'
