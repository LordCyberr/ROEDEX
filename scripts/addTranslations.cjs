...
for (const comp of Object.keys(NEW_ARRAYS)) {
  for (const lang of Object.keys(NEW_ARRAYS[comp])) {
    const keys = Object.keys(NEW_ARRAYS[comp][lang]);
    
    // Find the end of the `lang` block for this `comp`
    // It looks like `ru: { \n ... \n },`
    
    // Let's just do simple regex replacement to inject the new keys before the `}` of the lang block.
    ...
    let bracketCount = 1;
    let i = blockStart + searchString.length;
    ...
    const blockEnd = i - 1; // Position of the '}'
    
    let newProperties = '';
    for (const key of keys) {
      newProperties += `\n      ${key}: ${JSON.stringify(NEW_ARRAYS[comp][lang][key])},`;
    }
    
    content = content.slice(0, blockEnd) + newProperties + '\n    ' + content.slice(blockEnd);
  }
}

fs.writeFileSync(FILE_PATH, content);
console.log('Successfully injected new arrays!');
