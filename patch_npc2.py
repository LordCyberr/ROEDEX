...
import { Users, Crosshair, Search, MapPin, ChevronDown, ChevronRight } from 'lucide-react';
...
export const NPCView: React.FC = () => {
  ...
  return (
    <div className="flex-1 w-full flex flex-col p-4 overflow-hidden min-h-0 bg-[var(--bg-base)]">
      ...
      {zones.length > 0 && (
        <div className="grid grid-cols-[85px_1fr_25px] gap-1.5 px-3 py-1.5 mb-2 rounded bg-[var(--bg-panel)] border border-[var(--border-accent)] text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider shrink-0">
      ...
      <div className={`flex flex-1 min-h-0 ${isHorizontal ? 'flex-row overflow-x-auto gap-4' : 'flex-col overflow-y-auto gap-2 max-h-[250px]'} custom-scrollbar pb-2 pr-2`}>
        {zones.length === 0 ? (
          ...
          zones.map((zone, idx) => {
            ...
            return (
              <div key={idx} className={`flex flex-col ${isHorizontal ? 'w-[300px] shrink-0' : 'w-full'}`}>
                <button 
                  onClick={() => toggleZone(zone)}
                  className="flex items-center gap-1.5 px-1 py-1 mb-1 text-[11px] font-bold text-[var(--accent-primary)] uppercase tracking-wider hover:bg-white/5 rounded transition-colors"
                >
                  ...
                  <div className="flex flex-col gap-1 pl-1">
                    {groupedNpcs[zone].map(npc => {
                      ...
                      return (
                        <div 
                          key={npc.name}
                          className={`grid grid-cols-[85px_1fr_25px] gap-1.5 px-2 py-2 items-center rounded border transition-colors ${
                            isPinned 
