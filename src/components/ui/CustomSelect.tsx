...
export const CustomSelect: React.FC<CustomSelectProps> = ({ 
  ...
  return (
    <div className={`relative flex-shrink-0 ${className} ${disabled ? 'opacity-50 pointer-events-none' : ''}`} ref={ref}>
      ...
      {isOpen && (
        <div className={`absolute top-full right-0 mt-1 w-max min-w-full overlay-panel border-[var(--border-accent)] shadow-xl z-[100] max-h-[200px] overflow-y-auto overflow-x-hidden flex flex-col [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[var(--accent-primary)]/50 hover:[&::-webkit-scrollbar-thumb]:bg-[var(--accent-primary)] [&::-webkit-scrollbar-thumb]:rounded-full ${dropdownClassName}`}>
          {options.map((opt) => (
            <div 
              key={opt.value} 
