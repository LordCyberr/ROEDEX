...
def patch_npc():
    ...
    # Insert activeZone state
    if 'const [activeZone' not in content:
        insert_idx = content.find('const [searchTerm')
        if insert_idx != -1:
            content = content[:insert_idx] + "const [activeZone, setActiveZone] = useState<string>('');\n  " + content[insert_idx:]
    ...
    new_return = """// Determine zones to show on right side in horizontal mode
  ...
  return (
    <div className="flex-1 w-full flex flex-col p-4 overflow-hidden min-h-0 bg-[var(--bg-base)]">
      ...
      {isHorizontal ? (
                ...
                {zones.map(zone => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between text-[9px] font-black hover:text-white transition-colors uppercase tracking-widest px-2 py-1.5 border-b border-[var(--border-subtle)] mb-0.5 ${
              ...
              <div className="flex flex-row gap-2 overflow-x-auto custom-scrollbar flex-1 pb-2 h-full items-start">
                {horizontalZonesToRender.map(zone => (
                  <div key={zone} id={`npc-${zone.replace(/\\s+/g, '-')}`} className="flex flex-col w-[320px] max-w-full shrink-0 bg-[var(--bg-card)] rounded-lg shadow-md border border-[var(--border-subtle)] h-fit max-h-full">
                    ...
                    <div className="grid grid-cols-[85px_1fr_30px] gap-1 px-2 py-1 bg-[var(--bg-base)] border-b border-[var(--border-subtle)] text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-wider shrink-0">
                    ...
                    <div className="flex flex-col overflow-y-auto custom-scrollbar p-1 gap-1">
                      {groupedNpcs[zone].map(npc => {
                        ...
                        return (
                          <div 
                            key={npc.name}
                            className={`grid grid-cols-[85px_1fr_30px] gap-1 px-2 py-1.5 items-center rounded border transition-colors ${
                              isPinned 
        ...
        <>
          {zones.length > 0 && (
            <div className="grid grid-cols-[85px_1fr_30px] gap-1 px-3 py-1.5 mb-2 rounded bg-[var(--bg-panel)] border border-[var(--border-accent)] text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider shrink-0">
              <div className="truncate">{t('columns.npc')}</div>
              <div className="truncate">{t('columns.location')}</div>
              <div className="text-center truncate">{t('columns.track')}</div>
            </div>
                      ...
                      <div className="flex flex-col gap-1 pl-1">
                        {groupedNpcs[zone].map(npc => {
                          ...
                          return (
                            <div 
                              key={npc.name}
                              className={`grid grid-cols-[85px_1fr_30px] gap-1 px-2 py-2 items-center rounded border transition-colors ${
                                isPinned 
                                  ? 'bg-amber-500/10 border-amber-500/30' 
                                  : 'bg-[var(--bg-hover)] border-transparent hover:border-[var(--border-subtle)]'
                              }`}
